class Resident < ApplicationRecord
  ...
  def wb_contributions
    Worldbuilder::Contributor.joins(:district_user, :affiliate_user).includes(:district_resident).select("id", "district_id", "worldbuilder_districts.id", "worldbuilder_districts.name", "worldbuilder_districts.slug", "worldbuilder_districts.short_description", "worldbuilder_districts.updated_at").where("affiliation_id IN (?)", self.affiliated.map(&:id).uniq)
  end

  def cm_pc_campaigns
    Campaignmanager::Player.joins(:campaign_user).includes(:campaign, :campaign_resident).where("affiliation_id IN (?)", self.affiliated.map(&:id).uniq).order("campaignmanager_campaigns.name")
  end
  ...
  def campaign_list
    ...
    campaign_list.sort_by! { |dl| dl.name.downcase }
  end
  ...
  def adventure_list(core_rules)
    ...
    full_list.group_by { |x| x.type.demodulize.titleize.pluralize }# + stock_list
  end
