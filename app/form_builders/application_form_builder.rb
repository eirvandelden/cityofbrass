class ApplicationFormBuilder < ActionView::Helpers::FormBuilder
  TYPE_TO_FIELD = {
    ...
    datetime: :datetime_select, date: :date_select
  }.freeze
  ...
  def input(attribute, options = {})
    ...
    field_html =
      ...
      when :select, :grouped_select
        build_select(attribute, collection, options, field_type == :grouped_select)
      when :check_boxes
        collection_check_boxes(attribute, collection, :first, :last)
      else
        send(TYPE_TO_FIELD.fetch(field_type, :text_field), attribute, options)
      end

    wrap(attribute, field_type, required, label_text, wrapper_html) { field_html }
  end
  ...
  def association(attribute, options = {})
    ...
    field_html = collection_select(id_attr, collection, value_method, label_method,
                                    { include_blank: prompt }, options)
    wrap(id_attr, :select, required_attribute?(id_attr), options[:label], {}) { field_html }
  end
  ...
  def wrap(attribute, type, required, label_text, wrapper_html)
    ...
    label_html = build_label(attribute, type, state, required, label_text)
    ...
    @template.content_tag(:div, class: [ css, wrapper_html[:class] ].compact.join(" ")) do
      @template.safe_join([ label_html, yield, error_html ].compact)
    end
  end
