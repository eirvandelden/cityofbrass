    ...
    private
      ...
      def set_adventure
        if parent_type.include?"adventure"
          ...
          @adventure = @parent_object.adventure
        end
      end
      ...
      def set_entities
        @entities = ""
        @entities = current_user.resident.resident_npcs.where(core_rules: @adventure.core_rules).order_name                               if params[:entity_type] == "resident_npc"
