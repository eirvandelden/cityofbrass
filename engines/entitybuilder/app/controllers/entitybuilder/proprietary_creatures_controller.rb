...
module Entitybuilder
  class ProprietaryCreaturesController < EntitiesController
    ...
    private
      def set_type
        @type = 'ProprietaryCreature'
      end

      def set_entities
        if admin_signed_in?
          @entities = klass.includes([:gallery_image]).search(params[:search]).core_rules_filter(params[:core_rules_filter]).short.order_name.page(params[:page])
        else
          @entities = klass.includes([:gallery_image]).where(privacy: "Residents").search(params[:search]).core_rules_filter(params[:core_rules_filter]).short.order_name.page(params[:page])
        end
      end
