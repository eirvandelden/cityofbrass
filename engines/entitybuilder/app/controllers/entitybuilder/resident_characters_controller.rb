...
module Entitybuilder
  class ResidentCharactersController < EntitiesController
    ...
    private
      def set_type
        @type = 'ResidentCharacter'
      end

      def set_entities
        @entities = current_user.resident.send("#{@type.tableize}").includes([:class_levels, :gallery_image]).search(params[:search]).core_rules_filter(params[:core_rules_filter]).short.page(params[:page])
        ...
        if @entities.blank?
          @core_faq = Support::CoreFaq.joins(:faq).active.find_by_core_item('EB Character Index')
        end
      end
      ...
      def set_entity_for_show
        @entity = klass.joins(:user).includes([:campaign, :class_levels, :descriptors]).find_by_id(params_id)
        if @entity.nil?
          render template: 'errors/404', layout: 'layouts/application', status: 404
        end
        @parent_object = @entity
