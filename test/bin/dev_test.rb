...
class BinDevTest < ActiveSupport::TestCase
  test "bin/dev runs foreman with the development procfile" do
    ...
    assert_includes contents, 'exec bundle exec foreman start -f Procfile.dev -p "$PORT" "$@"'
    assert_not_includes contents, "bundle exec sidekiq"
    assert_not_includes contents, "trap"
